...
namespace WindowsFormsApp1
{
    public partial class Form1 : Form
    {
                    ...
                    try
                    {
                        ...
                        using (NpgsqlDataReader reader = npgsqlCommand.ExecuteReader())
                        {
                            ...
                            for (int i = 0; i < reader.FieldCount; i += 1)
                            {
                                ...
                                dataGridView2.Columns[i].Name = $"{reader.GetName(i).ToUpper()}, {reader.GetDataTypeName(i)}";
                            }
            ...
            using (conn = new NpgsqlConnection(Host_String))
            {
                ...
                using (NpgsqlCommand cmd = new NpgsqlCommand(sql, conn))
                {
                    try
                    {
                        ...
                        string logString = "SELECT * FROM jets WHERE jet_name='" + textBox7.Text + "' AND jet_count=" + textBox8.Text + " AND id_jets = (SELECT MAX(id_jets) FROM jets)";
                        NpgsqlDataAdapter logadapter = new NpgsqlDataAdapter(logString, conn);
                        logadapter.Fill(log);
                        dataGridView2.DataSource = log.Tables[0];
                    }
                    catch
                    {
            ...
            using (conn = new NpgsqlConnection(Host_String))
            {
                ...
                using (NpgsqlCommand cmd = new NpgsqlCommand(sql, conn))
                {
                    try
                    {
                        dataGridView2.Columns.Clear();
                        DataSet log = new DataSet();
                        string logString = "SELECT * FROM jets WHERE id_jets =" + textBox6.Text;
                        NpgsqlDataAdapter logadapter = new NpgsqlDataAdapter(logString, conn);
                        logadapter.Fill(log);
                        dataGridView2.DataSource = log.Tables[0];
                        cmd.ExecuteNonQuery();
        ...
        private void button9_Click(object sender, EventArgs e)
        {
            using (conn = new NpgsqlConnection(Host_String))
            {
                ...
                try
                {
                    ...
                    string queryString = "SELECT * FROM jets order by id_jets";
                    ...
                    //dataGridView2.Columns.Clear();
                    //DataSet log = new DataSet();
                    //string logString = "SELECT * FROM jets order by id_jets";
                    //NpgsqlDataAdapter logadapter = new NpgsqlDataAdapter(logString, conn);
                    //logadapter.Fill(log);
                    //dataGridView2.DataSource = log.Tables[0];
                }
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            ...
            {
                conn.Open();
                string sql = string.Format("UPDATE jets SET jet_name='" + textBox7.Text + "', jet_count=" + textBox8.Text + " WHERE id_jets=" + textBox6.Text, conn);
                using (NpgsqlCommand cmd = new NpgsqlCommand(sql, conn))                       //UPDATE jets SET jet_name='Aboba-4', jet_count=100 WHERE id_jets=142
                {
                    //try
                   // {
                        cmd.ExecuteNonQuery();
